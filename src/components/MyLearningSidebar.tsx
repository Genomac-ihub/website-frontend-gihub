...
export default function MyLearningSidebar({ isOpen, onClose, user, mode = 'overlay' }: Props) {
  const { pathname } = useLocation();

  const items = [
    { to: '/my-learning', label: 'Videos', icon: 'bi bi-play-btn' },
    { to: '/courses', label: 'Courses', icon: 'bi bi-book' },
  ...
  if (mode === 'push') {
    return (
      <aside
        className={`fixed left-0 top-0 h-full w-72 bg-[#151515] text-white shadow-2xl transform transition-transform duration-300 z-[40] ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}
      >
        <div className="px-6 py-6 space-y-6">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-full bg-orange-500 flex items-center justify-center font-bold">G</div>
          ...
          <nav className="space-y-2">
            {items.map(({ to, label, icon }) => {
              ...
              const baseClasses = 'flex items-center justify-between px-4 py-3 rounded-xl transition';
              ...
              return isClickable ? (
                <Link
                  ...
                  to={to}
                  onClick={onClose}
                  className={`${baseClasses} ${isActive ? 'bg-white text-black' : 'hover:bg-white/10'}`}
                >
                  <div className="flex items-center gap-3">
  ...
  return (
    <div className={`fixed inset-0 z-[70] ${isOpen ? 'visible' : 'invisible'}`}>
      ...
      <aside className={`absolute left-0 top-0 h-full w-72 bg-[#151515] text-white shadow-2xl transform transition-transform duration-300 ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="px-6 py-6 space-y-6">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-full bg-orange-500 flex items-center justify-center font-bold">G</div>
            <span className="text-lg">-IHub</span>
